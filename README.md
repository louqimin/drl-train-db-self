...
训了十六轮 D1 locomotion，「感觉都不行」，但**说不清哪一轮的哪个改动起了什么作用** ——
数据散在 `logs/` 目录、状态档表格和对话记录里，无法交叉查询。

典型要回答的问题：

- `feet_slide` 这一项在所有轮次里怎么变的，和步态指标是什么关系？
- kp 从 25 改到 50 那几轮，`dof_acc_l2` 的实测值差多少？
- 四腿 duty 极差比小于 2.0 的轮次，有什么共同点？
...
法则：子行离开父行还有没有意义。有就 RESTRICT，没有就 CASCADE。

训练记录离开机器人型号仍是独立观测（要拦）；奖励项离开跑次就是垃圾（跟着走）。
...
唯独帮不了「只按 b 查」。而「某个奖励项跨轮次怎么变的」正是本库最核心的问题。
